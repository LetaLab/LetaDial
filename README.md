...
- on Apache, every `storage/` subdirectory (including `thumbnails/`) is
  protected by a `Deny from all`-equivalent `.htaccess` - none of them
  are meant to be reachable by direct file URL, only through their
