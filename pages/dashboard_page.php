...
(function() {
    ...
    async function checkUpdate() {
        try {
            ...
            // SEC-147 (SEC_AND_BUG_ANIH_PLAN.md, Czesc XVI): data.url (the
            // release's html_url from GitHub's API, see
            // Updater::fetchLatestRelease()) was placed into href through
            // escHtml() alone, with no scheme allow-list - escHtml() only
            // neutralizes <, >, ", & and does nothing to stop a
            // javascript: (or other non-http(s)) scheme, unlike the rest
            // of this project, which consistently whitelists http/https
            // wherever a URL becomes a real link (dial_src.php::_validUrl(),
            // import_src.php::validateUrl(), bookmarklet_page.php's
            // FILTER_VALIDATE_URL usage). html_url is generated by GitHub
            // itself for the configured GITHUB_REPO, not free text a
            // third party can inject, so practical risk was always low -
            // this closes the inconsistency regardless, the same way
            // SEC-130 closed it for data.latest right next to it.
            const safeUrl = /^https:\/\/github\.com\//.test(data.url) ? data.url : '#';
            bannerText.innerHTML =
                `<strong>LetaDial ${escHtml(data.latest)} is available</strong>${escHtml(notes)} &nbsp;` +
                `<a href="${escHtml(safeUrl)}" target="_blank" rel="noopener noreferrer">View release →</a>` +
