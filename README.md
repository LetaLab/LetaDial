...
server {
    ...
    # ssl_stapling / ssl_stapling_verify / resolver removed 08.08.2026 - dead
    # config since Let's Encrypt fully retired OCSP responders in Aug 2025;
    # certificates issued since May 2025 no longer carry an OCSP responder
    # URL, so nginx has nothing left to staple. See "Otwarte pytania" #4 in
    # PLAN_NAPRAWY_BEZPIECZENSTWA_I_BUGOW_004.md.
...
The repository already ships with `/var/www/html/LetaDial/.htaccess` and `/var/www/html/LetaDial/storage/.htaccess` - both are tracked in git (unlike everything else under `storage/`, which `.gitignore` deliberately excludes), so a plain `git clone` or `Updater::gitPull()` already gives you both, with no manual step. Their content, reproduced below for reference:

`/var/www/html/LetaDial/.htaccess`:

```apacheconf
# Block ANY path segment starting with a dot, anywhere in the URL - the
# correct, path-aware way to protect .git/ under Apache (SEC-142 -
# <FilesMatch "^\."> below only matches the final filename, NOT the full
# path, so on its own it does not stop /.git/HEAD or /.git/objects/**).
RewriteEngine On
...
# redundant layer alongside the RewriteRule above.
<FilesMatch "^\.">
...
`/var/www/html/LetaDial/storage/.htaccess` - a single file at the `storage/` root; Apache applies `.htaccess` recursively down the directory tree, so this one file already protects `storage/thumbnails/`, `storage/avatars/`, `storage/group_icons/`, `storage/sessions/`, and any future subdirectory, with no per-directory copy needed:
...
If you are hand-configuring a server from a source that does not include these two tracked files (a non-git deployment, or verifying an existing install), create them yourself with the content above before going further.

`logs/.htaccess` needs the identical content to the `storage/.htaccess` block above, but is **not** git-tracked (it lives outside `storage/`). `install.php` creates it automatically on a fresh install, and `api/csp_report_api.php` also lazily recreates it the first time a CSP violation is logged if it's ever found missing - but if you're hand-configuring ahead of running the installer, create `logs/.htaccess` with that same content too.
...
LetaDial writes to `logs/` (CSP violation reports today, potentially more
in the future). The app itself only has one hard safety stop - writes to
`logs/csp-violations.log` are skipped once that file passes 10 MB - it
does not rotate or compress anything. For normal long-term operation you
should configure system `logrotate` so the directory never grows
unbounded between those stops.

### 1. Create `/etc/logrotate.d/letadial`

```bash
cat <<'EOF' > /etc/logrotate.d/letadial
/var/www/html/LetaDial/logs/*.log {
    daily
    rotate 14
    compress
    delaycompress
...
`stat -c '%U:%G' /var/www/html/LetaDial/logs` tells you the correct
owner/group if you are unsure.

### 2. Test it

```bash
logrotate -d /etc/logrotate.d/letadial
```

`-d` is a dry run - it prints what logrotate WOULD do without touching any
files. No separate cron entry is needed: logrotate already runs daily via
`/etc/cron.daily/logrotate` (or an equivalent systemd timer) on virtually
every Linux distribution, and will pick up this config automatically from
here on.
...
- `DB_HOST` should stay `localhost` (the installer's default) unless the
  network path to a remote database host is otherwise secured (VPN,
  private network, or TLS) - the PDO connection in `src/db_src.php` does
  not enable MySQL TLS by default
- `storage/` requires nginx `location ^~ /storage/ { deny all; }` - `.htaccess` is ignored by nginx
- on Apache, `storage/.htaccess` (tracked in git, ships with the repo) protects the
  entire `storage/` tree recursively from a single file - `thumbnails/`, `avatars/`,
  `group_icons/`, `sessions/` and any future subdirectory all inherit it automatically.
  None of them are meant to be reachable by direct file URL, only through their
  authenticated PHP endpoint (`Thumbnail::serve()`, `Avatar::serve()`, etc.)
- on Apache, `.git/` (and any other dot-prefixed path segment, anywhere in the URL)
  is blocked by a path-aware `RewriteRule` in the root `.htaccess`, not just the
  basename-only `<FilesMatch "^\.">` - the latter alone does not protect nested
  paths like `.git/HEAD` or `.git/objects/**` under Apache, a gap that would
  otherwise expose the full git history over plain HTTP
...
- Passwords: bcrypt cost 15, minimum 12 characters with complexity requirements
...
- `logs/csp-violations.log` writes stop past 10 MB as a hard backstop, but
  routine cleanup still needs system `logrotate` - see [Log rotation](#log-rotation)
