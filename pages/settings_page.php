    ...
    <div class="settings-section">
        ...
        <div class="settings-section-body">
            ...
            <div id="email-change-form" <?= $has_pending ? 'style="display:none"' : '' ?>>
                ...
                <div class="field-row">
                    <label class="form-label" for="email-change-pw">Current password</label>
                    <div class="input-wrap">
                        <input type="password" id="email-change-pw" class="form-input" autocomplete="current-password" placeholder="Confirm with your current password">
                        <button type="button" class="eye-btn" id="eye-btn-email-change-pw" aria-label="Show/hide"><?= eyeSvg(true) ?></button>
                    </div>
                    <div class="field-hint">Required to confirm this is really you — protects against a hijacked session changing your email.</div>
                </div>
...
// ── Change Email ───────────────────────────────────────────────────────────────
document.getElementById('btn-change-email')?.addEventListener('click',async()=>{hideAlert('email-alert');const newEmail=document.getElementById('new-email')?.value?.trim();const curPw=document.getElementById('email-change-pw')?.value||'';if(!newEmail){showAlert('email-alert','email-alert-msg','error','Please enter a new email address.');return;}if(!curPw){showAlert('email-alert','email-alert-msg','error','Please enter your current password to confirm this change.');return;}const btn=document.getElementById('btn-change-email');btn.disabled=true;btn.textContent='…';const r=await apiPost('/api/settings/email',{new_email:newEmail,current_password:curPw});btn.disabled=false;btn.textContent='Send confirmation email';const pwField=document.getElementById('email-change-pw');if(pwField)pwField.value='';if(!r.ok){showAlert('email-alert','email-alert-msg','error',r.error||'Could not send confirmation email.');return;}if(r.smtp_enabled&&r.email_sent){showAlert('email-alert','email-alert-msg','success','Confirmation email sent to '+newEmail+'.');}else{showAlert('email-alert','email-alert-msg','info','Email change recorded, but SMTP is disabled. Contact your admin.');}setTimeout(()=>location.reload(),3000);});
document.getElementById('eye-btn-email-change-pw')?.addEventListener('click',function(){togglePw('email-change-pw',this);});
