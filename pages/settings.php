<?php
/**
 * LetaDial — Settings Page (sesja 058 + 066)
 *
 * Sections:
 *   1. Change Password   → POST /api/settings/password
 *   2. Change Email      → POST /api/settings/email           (sesja 066)
 *   3. Two-Factor Auth   → POST /api/settings/backup-codes
 *   4. My Sessions       → GET/POST /api/settings/sessions    (sesja 066)
 *   5. UI Preferences    → localStorage + /api/settings/recent
 */
...
// Current session ID (to mark "this device" in sessions list)
...
.session-badge { font-size: .7rem; background: var(--primary); color: var(--primary-fg);
    padding: .1rem .5rem; border-radius: 9999px; font-weight: 700; margin-left: .4rem; }
...
.email-pending-banner .ep-icon { flex-shrink: 0; font-size: 1rem; }
...
<main class="settings-main">
    ...
    <!-- ══════════════════════════════════════════════════════════════
         SECTION 2: Change Email (sesja 066)
         ══════════════════════════════════════════════════════════════ -->
    <div class="settings-section">
        <div class="settings-section-header">
        ...
        <div class="settings-section-body">
            ...
            <div class="email-pending-banner">
                ...
                <div>
                    ...
                    <?php if (!$smtp_enabled): ?>
                    <br><em style="color:var(--warning)">Note: SMTP is disabled — the email was NOT sent. Run the SQL from Settings → Install Check or contact your admin.</em>
                    <?php endif; ?>
                </div>
            </div>
            <button type="button" class="btn btn-ghost btn-sm" id="btn-cancel-email-change" style="margin-bottom:.75rem">
                ✕ Cancel email change
            </button>
            <?php endif; ?>
            ...
            <div id="email-change-form" <?= $has_pending ? 'style="display:none"' : '' ?>>
                <div class="field-row">
                    ...
                    <div class="field-hint" style="color:var(--warning)">
                        ⚠ SMTP is not configured. The confirmation email cannot be sent.
                        Contact your admin to apply the change manually.
                    </div>
                    <?php else: ?>
                    <div class="field-hint">
                        A confirmation link will be sent to the new address. It expires in 1 hour.
                    </div>
                    <?php endif; ?>
                </div>
                <button type="button" class="btn btn-primary" id="btn-change-email">
    ...
    <!-- ══════════════════════════════════════════════════════════════
         SECTION 4: My Sessions (sesja 066)
         ══════════════════════════════════════════════════════════════ -->
    <div class="settings-section">
        <div class="settings-section-header">
            <span class="settings-section-icon">🖥️</span>
            <h2>Active Sessions</h2>
            <button type="button" class="btn btn-ghost btn-sm" id="btn-refresh-sessions"
        ...
        <div class="settings-section-body">
            ...
            <div style="margin-top:1rem;display:flex;justify-content:flex-end">
                <button type="button" class="btn btn-ghost btn-sm" id="btn-signout-all-others"
                        style="border-color:var(--error-bdr);color:var(--error)">
...
document.getElementById('btn-change-email')?.addEventListener('click', async () => {
    ...
    if (r.smtp_enabled && r.email_sent) {
        showAlert('email-alert', 'email-alert-msg', 'success',
            'Confirmation email sent to ' + newEmail + '. Click the link in the email to confirm the change.');
    } else if (!r.smtp_enabled) {
        showAlert('email-alert', 'email-alert-msg', 'info',
            'Email change recorded, but SMTP is disabled. Contact your admin to apply the change manually.');
    } else {
        showAlert('email-alert', 'email-alert-msg', 'info',
            'Email change recorded, but the confirmation email could not be sent. Contact your admin.');
    }
    // Reload after short delay to show the pending banner
...
document.getElementById('pref-recent-disabled')?.addEventListener('change', async function() {
    ...
    if (!r.ok) {
        ...
        this.checked = !disabled; return;
    }
    showAlert('pw-alert', 'pw-alert-msg', 'success',
        disabled ? 'Recent tab hidden. Reload the dashboard to apply.'
...
regenBtn?.addEventListener('click', () => {
    ...
    if (newCodesArea) newCodesArea.style.display = 'none';
    hideAlert('bc-alert');
    bcCodeInput?.focus();
...
bcCodeInput?.addEventListener('input', function() {
    ...
    if (this.value.replace(/\D/g,'').length === 6 || this.value.length === 9) confirmBtn?.click();
});
confirmBtn?.addEventListener('click', async () => {
    ...
    if (grid) {
        ...
        (r.backup_codes || []).forEach(c => {
            const el = document.createElement('div'); el.className = 'backup-code-item'; el.textContent = c; grid.appendChild(el);
        });
    }
...
function downloadCodes(codes) {
    ...
    const content = ['===========================================','  ' + appName + ' — 2FA Backup Codes','  Generated: ' + now,'===========================================','IMPORTANT: Each code can only be used ONCE.','Store this file in a password manager.','===========================================','', ...lines,'','===========================================','After using all codes, regenerate them in','Settings → Two-Factor Authentication.'].join('\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = appName.replace(/[^a-z0-9_-]/gi, '_') + '_backup_codes.txt';
...
function parseUA(ua) {
    ...
    let browser = 'Unknown', os = 'Unknown', icon = '🖥️';
    if (/EdgA?\//.test(ua))                           browser = 'Edge';
    else if (/OPR\//.test(ua))                         browser = 'Opera';
    else if (/Chrome\//.test(ua))                      browser = 'Chrome';
    else if (/Safari\//.test(ua) && /Version\//.test(ua)) browser = 'Safari';
    else if (/Firefox\//.test(ua))                     browser = 'Firefox';
    if      (/Windows NT/.test(ua)) { os = 'Windows'; icon = '🖥️'; }
    else if (/Macintosh/.test(ua))  { os = 'macOS';   icon = '🍎'; }
    else if (/Android/.test(ua))    { os = 'Android';  icon = '📱'; }
...
async function loadSessions() {
    ...
    const html = r.sessions.map(s => {
        ...
        const delBtn = isCur
            ? `<span style="font-size:.75rem;color:var(--text-faint)">current</span>`
            : `<button type="button" class="btn btn-ghost btn-sm"
                  style="border-color:var(--error-bdr);color:var(--error)"
                  onclick="deleteSession('${esc(s.id)}', this)">Sign out</button>`;
        return `<div class="session-item${isCur ? ' current-session' : ''}">
            ...
            <div class="session-info">
                ...
                <div class="session-meta">
                    IP: <strong>${esc(s.ip)}</strong> &nbsp;·&nbsp;
                    Last active: ${relTime(s.last_activity)} &nbsp;·&nbsp;
                    Signed in: ${relTime(s.created_at)}
                </div>
            </div>
...
async function deleteSession(sessionId, btn) {
    ...
    if (!r.ok) {
        ...
        showAlert('sess-alert', 'sess-alert-msg', 'error', r.error || 'Could not sign out session.');
        return;
    }
    showAlert('sess-alert', 'sess-alert-msg', 'success', 'Session signed out.');
    loadSessions();
}

document.getElementById('btn-refresh-sessions')?.addEventListener('click', () => {
    hideAlert('sess-alert');
    loadSessions();
});
...
document.getElementById('btn-signout-all-others')?.addEventListener('click', async () => {
    ...
    const n = r.deleted || 0;
    showAlert('sess-alert', 'sess-alert-msg', 'success',
        n === 0 ? 'No other sessions to sign out.' : `${n} other session${n !== 1 ? 's' : ''} signed out.`);
    loadSessions();
});
