...
// ── API ───────────────────────────────────────────────────────────────────────
// Extract only the FIRST path segment after /api/ to find the handler file.
// Example: /api/groups/123 → segment = 'groups' → api/groups.php
// The handler file itself parses the full URI for sub-routes (PUT/DELETE /id).
if (str_starts_with($uri, '/api/')) {
    $segment = explode('/', ltrim(substr($uri, 5), '/'))[0];
    $segment = preg_replace('/[^a-z0-9_-]/', '', $segment); // sanitize
